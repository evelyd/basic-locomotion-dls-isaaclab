from .common_cfg import CyberCommonCfg
import numpy as np
...
class CyberStandDanceConfig(CyberCommonCfg):
    ...
    # this would be defined in the asset i think
    ...
    # overwrite some of the vals in the commoncfg
    ...
    def __post_init__(self):
        # update the initial pose after setting the robot
        if init_pose == "sit":
            self.robot.init_state = SIT_INIT_STATE
            # Update randomization ranges for sitting if needed
            if "init_dof_pos_randomization" in self.events.terms:
                rand_cfg = self.events.terms["init_dof_pos_randomization"]
                rand_cfg.dof_pos_range_min = {k: v - 0.1 for k, v in SIT_INIT_STATE.joint_pos.items() if k != ".*"}
                rand_cfg.dof_pos_range_max = {k: v + 0.1 for k, v in SIT_INIT_STATE.joint_pos.items() if k != ".*"}
