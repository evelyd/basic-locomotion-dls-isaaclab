...
# Example: from .go2_robot_cfg import ALIENGO_CFG, ALIENGO_HIP_ACTUATOR_CFG, ALIENGO_THIGH_ACTUATOR_CFG, ALIENGO_CALF_ACTUATOR_CFG
...
@configclass
class Go2RewardCfg:
    """Reward terms for Go2StandDance environment."""
    ...
    base_height_reward: RewardTermCfg = RewardTermCfg( # This was previously orientation_penalty
        ...
        weight=1.0,  # Keeping original weight for base_height from Go2FlatEnvCfg
    )
...
@configclass
class Go2ObservationCfg:
    """Observation terms for Go2StandDance environment."""
...
@configclass
class Go2TerminationCfg:
    """Termination terms for Go2StandDance environment."""
...
@configclass
class Go2StandDanceEnvCfg(ManagerBasedRLEnvCfg): # <--- Inherit from ManagerBasedEnvCfg
    """Configuration for Go2StandDance environment in Isaac Lab."""

    # Curriculum settings
    # curriculum: bool = (INIT_POSE == "sit") # Adapt as needed for Go2
    cl_init: float = 0.6
    cl_step: float = 0.2
    ...
    scene: Go2StandDanceSceneCfg = Go2StandDanceSceneCfg()
    rewards: Go2RewardCfg = Go2RewardCfg()
    observations: Go2ObservationCfg = Go2ObservationCfg()
    terminations: Go2TerminationCfg = Go2TerminationCfg()
    events: Go2EventCfg = Go2EventCfg() # Changed from randomization to events
...
class Go2StandDanceDirectEnvCfg(DirectRLEnvCfg):
    ...
    rewards: MyRewardsCfg = MyRewardsCfg(
        scales={
            ...
            "collision": -2.0e2,
            "torque_limits": -0.01,
            "tracking_lin_vel": 0.8e1 * 5,
            "tracking_ang_vel": 0.5e1 * 5,
            # "rear_air": -0.5,
            "action_rate": -0.07e1,
