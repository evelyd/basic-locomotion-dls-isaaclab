...
class AliengoStandDanceEnv(SymmlocoCommonEnv):
    ...
    def _update_command_curriculum(self, env_ids: torch.Tensor, extras: dict):
        ...
        if normalized_ang_vel_reward > 0.8:
            self._command_ranges["ang_vel_z"][0] = torch.clip(self._command_ranges["ang_vel_z"][0] - 0.2, -self.cfg.command_max_curriculum, 0.)
            self._command_ranges["ang_vel_z"][1] = torch.clip(self._command_ranges["ang_vel_z"][1] + 0.2, 0., self.cfg.command_max_curriculum)
